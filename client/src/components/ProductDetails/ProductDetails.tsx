...
const ProductDetails: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchSameCategory = async () => {
      try {
        const res = await axios.get(`http://localhost:3005/api/products/category/${product.category}`);
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await axios.get("http://localhost:3005/api/products/categories");
            ...
            {allImages.map((img, i) => (
              <img
                ...
                className={`w-16 h-16 object-cover rounded-md cursor-pointer border-2 ${
                  i === imageIndex ? "border-yellow-500" : "border-transparent"
                }`}
              />
            ))}
          </div>
          ...
          {cookies.role !== "admin" && (
            <button
              ...
              className={`px-6 py-3 rounded-md text-lg font-semibold transition ${
                selectedVariant.stock === 0 || isInCart
        ...
        <div className="flex gap-3 mb-6 flex-wrap">
          <button
            ...
            className={`px-4 py-2 rounded-md border ${
              selectedCategory === "All" ? "bg-yellow-500 text-white border-yellow-500" : "bg-white text-gray-800 border-gray-300 hover:border-yellow-400"
            }`}
          >
            All
          </button>
          {categories.map((cat) => (
            <button
              ...
              className={`px-4 py-2 rounded-md border ${
                selectedCategory === cat ? "bg-yellow-500 text-white border-yellow-500" : "bg-white text-gray-800 border-gray-300 hover:border-yellow-400"
              }`}
            >
              {cat}
            </button>
